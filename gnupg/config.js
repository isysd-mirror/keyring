...
export function createGnuPGConfig (superclass) {
  ...
  return class GnuPGConfig extends createGnuPG(superclass) {
    ...
    async getConfig (file = 'gpg.conf') {
      var p = path.join(this.home, file)
      var lines = await fs.promises.readFile(p, 'utf-8')
      this._config = expandConfig(lines.split('\n'))
      return this._config
    }

    async setConfig (key, vals = [], comments = '', file = 'gpg.conf') {
      var p = path.join(this.home, file)
      var lines = await fs.promises.readFile(p, 'utf-8')
      var re = `[#]*${key}.*[\n]+`
    ...
    async writeConfigFile (contents, file = 'gpg.conf') {
      return fs.promises.writeFile(path.join(this.home, file), contents)
    }

    async setConfigDefaults (file = 'gpg.conf') {
      Object.keys(DEFAULTS[file]).forEach(d => {
        this.config[d] = DEFAULTS[file][d]
      })
      return this.writeConfigFile(flattenConfig(this.config), file)
    }
